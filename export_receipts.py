...
async function exportToExcel() {
    ...
    if (typeof receiptsData !== 'undefined' && receiptsData.length > 0) {
        receiptsData.forEach(tx => {
            const itemsStr = tx.items.map(i => `${i.qty}x ${i.desc} (₱${i.subtotal})`).join('\\n');
    ...
    summaryRRow.getCell(5).numFmt = '₱#,##0.00';
...
new_code = re.sub(r'async function exportToExcel\(\)\s*\{[\s\S]*?URL\.revokeObjectURL\(url\);\n\}', new_export_code.strip(), code)
