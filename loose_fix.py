            ...
            div.innerHTML = `
                <div class="history-content">
                    ...
                    <div class="history-meta" style="margin-top: 0.5rem; display: flex; justify-content: space-between; align-items: center;">
                        ...
                        <button class="btn btn-secondary print-past-receipt-btn" style="padding: 0.25rem 0.5rem; font-size: 0.75rem;"><i class="ri-printer-line"></i> Print Receipt</button>
                    </div>
                </div>
            `;
...
new_js, count = re.subn(r'if\s*\(\s*item\.type\s*===\s*\'sale\'\s*\)\s*\{.*?\}\s*else\s*if\s*\(\s*item\.type\s*===\s*\'transfer\'\s*\)', replacer, js, flags=re.DOTALL)
