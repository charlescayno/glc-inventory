...
// Make functions global for inline access if needed, though they shouldn't be
...
function saveReceipts() {
    ...
    // Save to Firebase
    setDoc(doc(db, "receipts", "main"), {
        data: receiptsData
    }).catch(e => console.error("Error saving receipts to Firebase", e));
}
...
function saveData() {
    ...
    setDoc(doc(db, "inventory", "main"), {
        data: inventoryData
    }).catch(e => console.error("Error saving inventory to Firebase", e));
}
...
function updateSummaries() {
    ...
    if (totalValuationEl) {
        totalValuationEl.textContent = 'Γé▒ ' + totalValuation.toLocaleString();
    }
}
...
function saveHistory() {
    ...
    // Save to Firebase
    setDoc(doc(db, "history", "main"), {
        data: historyData.slice(0, 100)
    }).catch(e => console.error("Error saving history to Firebase", e));
}
...
function parseHistoryEntry(rawItem) {
    ...
    if (type === 'sale' && (!items || totalAmount === undefined) && item.desc) {
        const saleMatch = item.desc.match(/Deducted:\s*(.*?)\s*\|\s*Total:\s*Γé▒?([\d,]+)/i);
        if (saleMatch) {
            ...
            div.innerHTML = `
                ...
                <div class="history-content">
                    ...
                    <div class="history-route" style="margin-bottom: 0.35rem;">
                        <span class="history-qty-pill sale">Γé▒ ${(item.totalAmount || 0).toLocaleString()}</span>
...
function updateCartUI() {
    ...
    cartBarTotalEl.textContent = 'Γé▒ ' + totalBill.toLocaleString();
...
function renderCartModal() {
    ...
    if (activeEntries.length === 0) {
        ...
        cartGrandTotalEl.textContent = 'Γé▒ 0';
    ...
    activeEntries.forEach(([idStr, qty]) => {
        ...
        row.innerHTML = `
            <div class="cart-item-name">
                ...
                <div class="cart-item-unit-price">Γé▒ ${item.price} each (${locName} stock: ${item[checkoutLoc] || 0})</div>
            </div>
            ...
            <div class="cart-item-subtotal">Γé▒ ${subtotal.toLocaleString()}</div>
        `;
        cartItemsContainer.appendChild(row);
    });

    cartGrandTotalEl.textContent = 'Γé▒ ' + grandTotal.toLocaleString();
    updateChangeDue();
    ...
    cartItemsContainer.querySelectorAll('.cart-modal-qty-input').forEach(input => {
        input.addEventListener('input', (e) => {
            ...
            cartGrandTotalEl.textContent = 'Γé▒ ' + modalTotal.toLocaleString();
            updateChangeDue();
        });
        input.addEventListener('focus', function() { this.select(); });
        input.addEventListener('click', function() { this.select(); });
...
function updateChangeDue() {
    ...
    if (cashInputVal === '') {
        changeDueDisplay.textContent = 'Γé▒ 0';
    ...
    if (diff >= 0) {
        changeDueDisplay.textContent = 'Γé▒ ' + diff.toLocaleString();
        changeDueDisplay.classList.remove('short');
        if (changeLabelEl) changeLabelEl.textContent = 'Change Due';
        if (changeStatusBadge) {
            changeStatusBadge.textContent = diff === 0 ? 'Γ£ô Exact Amount' : 'Γ£ô Change Ready';
            changeStatusBadge.className = 'change-status-pill paid';
        }
    } else {
        const shortAmt = Math.abs(diff);
        changeDueDisplay.textContent = '- Γé▒ ' + shortAmt.toLocaleString();
        changeDueDisplay.classList.add('short');
        if (changeLabelEl) changeLabelEl.textContent = 'Short / Balance';
        if (changeStatusBadge) {
            changeStatusBadge.textContent = `Needs Γé▒ ${shortAmt.toLocaleString()} more`;
...
function checkoutCartDeduct() {
    ...
    const checkoutLoc = 'booth';
    const locName = 'GLC Booth';
    ...
    receipt.items.forEach(item => {
        html += `
            <div class="receipt-item">
                ...
                <div>Γé▒${item.subtotal.toLocaleString()}</div>
            </div>
        `;
    });
    ...
    html += `
        ...
        <div class="receipt-totals">
            <div class="receipt-total-row">
                <span>TOTAL DUE</span>
                <span>Γé▒${receipt.total.toLocaleString()}</span>
            </div>
            <div class="receipt-total-row sub">
                <span>CASH</span>
                <span>Γé▒${receipt.cash.toLocaleString()}</span>
            </div>
            <div class="receipt-total-row sub">
                <span>CHANGE</span>
                <span>Γé▒${receipt.change.toLocaleString()}</span>
            </div>
        </div>
        <div class="receipt-divider"></div>
...
async function exportToExcel() {
    ...
    sortedData.forEach(item => {
        const total = (item.floor5 || 0) + (item.floor7 || 0) + (item.booth || 0);
        const row = worksheet.addRow([item.category, item.desc, item.floor5 || 0, item.floor7 || 0, item.booth || 0, total]);
        for(let i=3; i<=6; i++) row.getCell(i).alignment = { horizontal: 'center' };
    });
    ...
    worksheet.getColumn(1).width = 30; worksheet.getColumn(2).width = 45;
    for(let i=3; i<=6; i++) worksheet.getColumn(i).width = 15;
    ...
    if (typeof receiptsData !== 'undefined' && receiptsData.length > 0) {
        receiptsData.forEach(tx => {
            ...
            const row = receiptSheet.addRow([new Date(tx.date).toLocaleString(), tx.id, tx.location, itemsStr, tx.total, tx.cash, tx.change]);
            row.getCell(4).alignment = { wrapText: true };
            row.getCell(5).numFmt = 'P#,##0.00'; row.getCell(6).numFmt = 'P#,##0.00'; row.getCell(7).numFmt = 'P#,##0.00';
    ...
    receiptSheet.getColumn(1).width = 22; receiptSheet.getColumn(2).width = 18; receiptSheet.getColumn(3).width = 15;
    receiptSheet.getColumn(4).width = 60; receiptSheet.getColumn(5).width = 15; receiptSheet.getColumn(6).width = 15;
    receiptSheet.getColumn(7).width = 15;
